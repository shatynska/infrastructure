...
| `ssh_allowed_cidrs` | The public IP ranges allowed to reach SSH. Must not be `0.0.0.0/0`. If everyone will use Tailscale, see `docs/backlog.md` entry 8 for closing public SSH entirely | one ISP `/24` | The same |
...
| `test` A | `2.29.14.98` — a throwaway smoke test's leftover; `docs/backlog.md` entry 22 covers removing it, and the certificate Traefik still renews for it |
...
**Revisit when** staging acquires its hostnames — `docs/backlog.md` entry 30, which is the first time the manual edit would be made twice — or when mail moves off this zone, or when a second hostname makes the manual edits frequent enough to be worth the risk. What the deferral costs meanwhile is real and worth stating: DNS is the one piece of the running system that lives in no repository, so a rebuild (`docs/backlog.md` entry 12) or an IPv4 change is followed by a manual edit, and this table is the mitigation.
