...
**Both of those spell the deploy target as the stack**, because the thing a deploy key authorises is an entry in `ansible/inventory/host_vars/<server name>.yml` — the vars file of the one host that stack provisions. Filename, comment, authorising entry and stored secret all sit on the stack now; the platform key's private half was always there, in that stack's GitHub Environment as `PLATFORM_DEPLOY_SSH_KEY`, resolved by `platform-deploy.yml`'s deploy job for that stack.

They spelled the *environment* until `deploy_apps` moved off it, and the history is worth a sentence because a reader may still meet it. While that list sat in an environment's `group_vars`, an entry authorised its key on every host in that environment, so a filename spelling the stack would have claimed a narrower blast radius than the key had. `bound-a-deploy-key-to-one-host-when-an-environment-holds-two-stacks` corrected the shape rather than the name, and the names followed. No key was re-issued: while each environment holds one stack the two spellings name the same keypair, and a key is identified by its fingerprint.

**The table below carries the keys a deployment assembles once.** An application's deploy key is not among them: there is one per application per stack and it is generated when that application is onboarded, which is a different occasion and usually a much later one — `docs/onboard-an-application.md` carries that command beside the step that runs it.
...
Covers stages 0 to 6. Two things sit outside it deliberately: stage 7's platform-stack secrets, a set of nine per stack, listed at §7.3 and Appendix A — `PLATFORM_DEPLOY_HOST` and `PLATFORM_DEPLOY_SSH_KEY` are created at stage 6.4 and belong to that same set; and the application deploy key, one per application per stack, which is generated at onboarding time by `docs/onboard-an-application.md` rather than here.
...
| Platform deploy keypair | **2** — one per stack | Each host's own vars file carries a `platform` entry in `deploy_apps`, and the two public keys differ. Keypair, name, authorising entry and stored private half are all per stack; the private half lives in that stack's own GitHub Environment |
...
- **Hetzner volume names are unique per project, not globally.** In one project the second stack's volume cannot also be called `main`, so it gets a different name — and since a mount path is chosen per stack rather than derived from a volume's name, a different mount path, so that `platform/docker-compose.yml`'s hardcoded `/mnt/main/prometheus`, `/mnt/main/grafana` and `/mnt/main/postgres` must be parameterised — or those services come up writing to a path that does not exist, silently. The third is the shared PostgreSQL instance, and it fails worse than the other two: Prometheus and Grafana come up empty, where the instance comes up **initialised** empty, holding none of the application databases it held the moment before, with `docker compose up -d --wait` reporting success.
...
| `ssh_allowed_cidrs` | The public IP ranges allowed to reach SSH. Must not be `0.0.0.0/0`. If everyone will use Tailscale, see `docs/backlog.md` `close-public-ssh-and-manage-sshd-explicitly` for closing public SSH entirely | one ISP `/24` | The same |
...
**Revisit when** mail moves off either zone, when a server's address changes — the moment its records are edited by hand — or when names outside the server wildcards become frequent enough that the manual edits recur. A new application hostname is not itself a trigger: under a server's wildcard it costs no edit. What the deferral costs meanwhile is real and worth stating: DNS is the one piece of the running system that lives in no repository, and since these tables hold no values the only copies here are the archive's stale readings — so a rebuild that changes a server's address (`docs/backlog.md` `write-and-rehearse-the-rebuild-runbook`) or an IPv4 change is followed by a manual edit to every record pointing at that server, found by reading the zones and not by reading this.
...
- **It repeats per application per deploy target**, not once per server. The infrastructure side sits on the host axis: `deploy_apps` lives in `ansible/inventory/host_vars/<server name>.yml`, so an application's deploy key, its `authorized_keys` line and its `sudoers` rule are one set per application per host.
...
**One row was set from intent rather than from observation, and it has since been read.** At that reading `main-staging-alertmanager` **did not exist** — both hosts were feeding production's check, which is what `give-staging-its-own-dead-mans-switch-check` was opened and closed on the same evening. Staging's row below was therefore written from production's observed values rather than from its own, and this paragraph went on to infer the rest from the preamble above: a check created by its own first ping comes into existence on the vendor's default, so staging's had to be sitting on one. **That inference was wrong.** The operator read the check on 2026-09-17 and found period **5 minutes**, grace **2 minutes**, green and receiving pings — the pair the row below already recorded. Nothing was changed at the observer, and how the check came to carry those values rather than the vendor's is not established. What the preamble licenses is the suspicion that a check *may* be sitting on a default, which is why it says to read these back; it does not license writing down what the default would have made of it, which is what this paragraph did for a day. The row is a record now because someone looked. The rehearsal's own comparison was made **before** it destroyed anything, which is what makes the observer the authority there: a disagreement found afterwards would more likely be the rebuild having reset a check to the vendor's default, and `docs/runbook-rebuild.md`'s phase 12 says so and corrects in the other direction.
...
**The per-stack Alertmanager checks are addressed by a different credential and so have a table of their own**, below rather than above: a ping URL each, held as that stack's `PLATFORM_DEADMANSWITCH_URL`, never the project ping key. That is why the table above says "the checks **it** addresses" and why the `HEARTBEAT_PING_KEY` row counts four. Count all of them against the free tier's twenty together: two stacks is six checks, not four.
...
**The sequence lives in `docs/runbook-rebuild.md`**, which gives it in order, names the credential each step needs and where it is held, and carries the date of its last rehearsal. It covers either host. This appendix used to hold that sequence in a paragraph, and kept one claim that was true of one route and false of the other for as long as it did — which is the drift a second account costs, and why there is now one.

What belongs here rather than there, because it is a claim about this document's own stages:
...
**Most of this is performed by a stage of this document, and the reason to list all of it in one place is the rest.** The last column names the stage that already instructs the edit; **`nothing`** means no stage does, and the copier makes that edit from this table. Read it after §3.1 and before the first converge — everything in it is committed content, so none of it needs a host.

A copy that follows §3.1 keeps the repository name `infrastructure` and the stack names `main-production` and `main-staging`. That is deliberate and costs nothing anywhere else, but it means every value derived from those names is **identical** between the two repositories rather than distinct, which is what the heartbeat rows are about.

| What a copy inherits | Where | What it must become | Performed by |
|---|---|---|---|
| The HCP organisation | `terraform/stacks/main-production/versions.tf` and `main-staging/versions.tf` | the company's own, from stage 2 | §3.1 step 3 |
| `company:` | `ansible/inventory/group_vars/all.yml` | the company's own short name, which becomes the first segment of every host's own name | §3.1 step 3 |
| `ghcr_pull_username` | `ansible/inventory/group_vars/production.yml` and `staging.yml` | the account whose token pulls that company's images | §3.1 step 3 |
| The `Documentation=` URL | `ansible/roles/image_prune/tasks/main.yml` | the company repository's URL | §3.1 step 3 |
| Sizing, location, the operator's public key, `ssh_allowed_cidrs`, labels | both `terraform.tfvars` | that company's §1.3 decisions, its own operator key and its own address ranges | §3.1 step 4 |
| The `moved` block | `terraform/stacks/main-production/ssh_key.tf` | deleted | §3.1 step 6 |
| `deploy_apps`, and every `public_key` and `probe_public_key` in it | `ansible/inventory/host_vars/main-production.yml` and `main-staging.yml` | **both files deleted**, and recreated at §6.1.1 | nothing |
| The two Vault-encrypted blocks, `ghcr_pull_token` and `image_prune_heartbeat_ping_key` | `ansible/inventory/group_vars/production.yml` and `staging.yml` | the company's own values, encrypted under vault passwords of its own | §6.1, once the inherited blocks are gone |
| Every heartbeat check slug | `.github/workflows/drift.yml`, `.github/workflows/pre-commit-autoupdate.yml`, and `ansible/roles/image_prune`'s templated one | unchanged — what makes them distinct is a heartbeat project of the company's own | §7.1, with the paragraph below |
| Status | `README.md` | what is true of that tree on its first day, which is nothing yet | nothing |
| The rehearsal record, and three commands naming this operator's hostnames | `docs/runbook-rebuild.md` | `Last rehearsed: never`, and the company's own hostnames | nothing |
| The entries naming this operator's own applications and third parties | `docs/backlog.md` | deleted; every other entry carries over unchanged | nothing |
| A constant naming an application the copy does not have | `.github/tests/` | moved with the thing it names | nothing |

**The `host_vars` files are the sharpest of these, and §6.1.1 does not reach them.** That section says the file "does not exist until you create it", which is true of a fresh repository and false of a clone: both files arrive with the tree, each carrying a `platform` entry and a `commerce-ops` entry whose public halves authorise keypairs whose private halves are held in *this* operator's GitHub Environments and in *this* operator's application repository. A company host converged against an inherited file authorises this operator to deploy to it, and nothing about that state looks wrong on the host. Delete both files before the first converge and let §6.1.1 create them: `.github/tests` fails a pull request where a stack's declared server has no file, so deleting them cannot be the thing that is forgotten.

**The Vault-encrypted blocks fail loudly instead, which is the one mercy in this list.** They are encrypted under vault passwords the copy does not have, so a converge that meets them stops rather than proceeding on someone else's values. Replace both at §6.1 rather than trying to decrypt them.

**The heartbeat slugs are literals, not names derived from the repository** — the paragraph that used to stand at the end of this appendix said otherwise, and it was wrong. `drift.yml` pings `infrastructure-drift` and `pre-commit-autoupdate.yml` pings `infrastructure-pre-commit-autoupdate`, both written out in the workflow; the host slug is `<inventory_hostname>-prune-host-images`, templated from a name §3.1 keeps. So a company copy following §3.1 produces **the same six slugs this repository does**. They collide only inside one heartbeat project, so the answer is a project of the company's own: its own `HEARTBEAT_PING_KEY` (§7.1), and a `PLATFORM_DEADMANSWITCH_URL` per stack created in that project (§7.3). The free tier's twenty checks is the ceiling either way — two stacks cost six of them, so count before adding a third host, and give a host an `inventory_hostname` of its own where two ever have to share a project.

**`README.md`'s Status and the rebuild runbook's rehearsal record are claims about a running host, and a copy inherits them as claims it cannot support.** Status says the bootstrap is done and two servers run, with their sizes; `docs/runbook-rebuild.md` carries a rehearsal date against a stack in this operator's Hetzner project, and three commands addressing this operator's hostnames. `.github/tests` holds that record to three states and *never* is one of them — `Last rehearsed: never`, with no `Partial run:` block — so write that, and set it when the company's own rehearsal happens. Rehearsing the rebuild on the company's staging stack is what makes its production build the second one rather than the first.

**`docs/backlog.md` carries over, and that is a recommendation rather than a default.** Its entries are about this code, and a copy inherits the code and its gaps together — the durable-data and backup boundary, container resource limits, a reboot policy, deploy-key revocation. Delete the entries naming this operator's own applications and third parties; keep the rest, and take the first three before real data lands on the company host.

**`openspec/` carries over whole**, specifications and archive alike: the specifications are what the static suite holds the tree to, and the archive is why each file is the way it is — a copy that drops it keeps every rule and loses every reason. `openspec/changes/` is empty at the fork and stays so until the company's first change. **Two things follow from keeping the archive.** It records this operator's server names, addresses and a tenant's application, which is one of the reasons §3.1 step 1 makes the company repository private. And a few constants in `.github/tests/` name that application — `UNMET_STORE` in `test_the_rebuild_runbook_records_its_rehearsal.py` is the one to expect — so a copy that removes an application from the rebuild runbook edits, in the same change, the assertion that required it to be named there.

### Renaming a server afterwards
