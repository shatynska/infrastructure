...
## 59. find-out-whether-a-failed-idempotence-check-can-pass-as-green

**Not blocked. Recorded 2026-09-16 from an observation, not from a reproduction — which is the whole reason it is an entry rather than a fix.**

A concurrent session running `run-molecule test -s probe-and-window` in another working tree reported that Molecule printed, after its final destroy:

    ERROR    Idempotence test failed because of the following tasks:
    *  => deploy_user : Install the privileged probe script

while **the process exited 0 and the SCENARIO RECAP said `failed=0`**. If that is what happened, then an idempotence failure passes both of the checks this repository's conventions tell a reader to make: `AGENTS.md`'s testing section says to distrust the exit code and read the recap instead, and here the recap agreed with the exit code and both disagreed with the ERROR. `.github/workflows/ansible-verify.yml` runs `../../scripts/run-molecule test --all` as a plain step and reads the exit status alone, so CI would pass it too.

**What is established.** The run aborted at `idempotence` and ran only its teardown afterwards: its `DETAILS` block lists ten actions — dependency, cleanup, destroy, syntax, create, prepare, converge, idempotence, cleanup, destroy — where that scenario's sequence declares twelve, the missing two being `side_effect` and `verify`. So **that run never executed a single one of the scenario's assertions**, and the session that made it withdrew its report accordingly. The idempotence failure itself is real rather than a reporting artefact; Molecule did what a failure makes it do. What is anomalous is only the last mile: **the process exited 0 and the recap said `failed=0` on a sequence that aborted two actions short of its end.**

**The cause of that particular failure is settled and is not a defect in the role.** `files/app-probe` was edited at 05:44:42, between that run's two converges, by another session working in the same tree — so the second converge copied a different file from the first and correctly reported `changed`. Twenty Molecule logs from runs of the same role with nobody editing carry no `Idempotence test failed` line at all. Two claims, and only the first is open: the edit explains why idempotence failed; nothing yet explains why an aborted sequence exits 0.

**Two count-based detectors suggest themselves; one is wrong and the other is merely the wrong thing to build.** This is recorded because the entry exists to stop an afternoon being spent, and "counting is wrong" would be the wrong lesson.

- *Executed count against a fixed twelve* is the wrong one. `ghcr-credential-rejected` reports `actions=10 successful=7 missing=4` — the same numbers as the aborted run — because its `molecule.yml` deliberately omits `idempotence` and `side_effect` from `test_sequence`, with a comment saying why: its converge aborts the role partway through on purpose, so idempotence there would fail for a reason unrelated to the scenario, and role idempotence is covered by `default` and `ghcr-credential-absent` instead. A fixed comparison fails that scenario for being correctly configured.
- *Executed count against each scenario's own declared sequence* survives that: ten declared and ten executed is equal, while the aborted run executed ten against twelve. It is not refuted, and it would have caught the run. It is still not the thing to build, because **`probe-and-window` declares no `test_sequence` at all** — its expected count is Molecule's own default, so a detector would have to carry a copy of that default in this repository and keep it true across an upgrade of the pin, which is a second thing to maintain and a silent failure when it drifts.

**What both are proxies for is whether anything was asserted, and that is directly observable.** `ghcr-credential-rejected`'s `DETAILS` carries `verify: Executed: Successful`; the aborted run's carries no `verify` line at all. A scenario holding a `verify.yml` that did not reach `verify` has established nothing, whatever it exited with — and that property needs no copy of anything, states the thing we care about rather than standing in for it, and is what `ansible/scripts/run-molecule` should refuse on.

**What to do.** Reproduce the exit status first — induce a failure at `idempotence` in a throwaway scenario and read the exit status, the recap and the `DETAILS` block against each other. If an aborted sequence really does exit 0, the fix belongs in `ansible/scripts/run-molecule`, which is where this repository already wraps Molecule: refuse a run whose `DETAILS` names a `verify.yml`-carrying scenario without a successful `verify`, rather than teaching every reader and every workflow to grep for prose. If it does not, delete this entry and say so.

**Adjacent, and the reason this is worth more than its size.** `AGENTS.md` already records that `molecule test --all` stops at the first failure and that the recap must be read rather than the exit code. That guidance exists because a run can report success having executed less than it claims. This entry asks whether there is a second, worse case of the same shape: a run that executed everything and reported success having failed.
