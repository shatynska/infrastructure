...
Note the divergence this does **not** cover: `commerce-ops` runs its own PostgreSQL container with durable data in it (entry 19). Nothing here touches that one, and this entry must not be read as a template for it -- discarding *that* volume loses data.
...
What it did not supply is anything to serve, and `onboard-commerce-ops-to-staging` has moved that only partway. **Staging has the platform stack and an authorised application that has not yet deployed.** `deploy_apps` in `ansible/inventory/group_vars/staging.yml` now enumerates `commerce-ops` beside `platform`, so the next converge gives it an `/opt` directory, a forced-command `authorized_keys` line and a sudoers rule — but nothing is deployed behind them, so Traefik there still routes nothing and still declares no router for ACME to answer for. Opening 80 and 443 to the public internet today would still put a reverse proxy with no backend there, and would still issue no certificate.

**So the trigger is an application reaching staging, not a date** — and it is approaching rather than met. What is outstanding, in order: the deploy itself, which needs `provision-commerce-ops-database-in-the-shared-instance` and a staging deploy path in the application's own repository; then the firewall opened and one DNS A record, which is this entry. **Opened at both layers together, and the reason is now narrower than it was**: for Traefik's own ports UFW is not in the path at all (see the paragraph below), so the cloud firewall is what actually gates them — but the mirror obligation between `terraform.tfvars` and `group_vars` stands for every port where UFW *is* the gate, and leaving the two files disagreeing is how a reader learns the wrong rule. Taking it before the deploy would still mean writing a dummy router to have something for ACME to answer for — a contrivance that rehearses the mechanism against a case production will never run.

**The tailnet already reaches those ports, and that is not what this entry is about.** Traefik publishes `0.0.0.0:80` and `0.0.0.0:443` on staging today, and a request from any tailnet peer is answered — measured on 2026-09-13 from the operator's workstation: `curl -H 'Host: …' http://100.85.219.36/` returns `301` to `https`, and `curl -k https://100.85.219.36/` returns `404` behind Traefik's own default certificate. `hardening_web_allowed_cidrs: []` does not prevent it and never did: Docker DNATs a published port in `nat/PREROUTING` and accepts it in `FORWARD`, while UFW's rules hang off `INPUT`, so a container-published port is never offered to them on any interface. `docs/backlog.md` entry 53 records that, because it is wider than this entry. What this entry is still about is the **public** internet, where the Hetzner cloud firewall is what refuses — and the certificate, which needs Let's Encrypt to connect inbound to 443 from outside.

**One consequence of the wait, so it is not chased as a fault.** Once an application's router does exist on staging, Traefik begins a TLS-ALPN-01 issuance it cannot complete — Let's Encrypt answers that challenge by connecting inbound to 443 from the public internet, where the cloud firewall refuses it — and retries on its own schedule, logging failures until this entry lands. Expected, and harmless to the deploy.
...
Measured from the operator's workstation, over the tailnet, against staging — whose `hardening_web_allowed_cidrs` is `[]` and whose `web_allowed_cidrs` is `[]`, and whose UFW is `active`:
...
**What this falsifies, and it is the reason this is an entry rather than a note.** The firewall convention in `AGENTS.md` and `README.md` says the two layers are the cloud firewall and UFW, and that for any given port exactly one of them is the documented access gate. For a container-published port that is not a split at all — there is one layer, and a reader who closes UFW and believes the port shut is wrong. `docs/bootstrap-a-new-host.md` says "both firewall layers refuse inbound traffic to them" in two places; `docs/backlog.md` entry 17 carries the two-layer obligation as one of the two things it says exist nowhere else; and `ansible/inventory/group_vars/staging.yml`'s comment above `hardening_web_allowed_cidrs` reasons from it directly — "staging's cloud firewall opens no web port at all, so UFW opens none either", which is true as a mirror of `terraform.tfvars` and misleading as a statement about what is reachable. Each needs to say which ports it is true of. The mirror obligation itself survives all of this: `terraform.tfvars` and `group_vars` still have to agree, and the reason is unchanged for every port UFW does gate.
...
**What is not owed here.** Nothing about this is an exposure: staging reaches no public web port, and a tailnet peer is an authenticated device of the operator's own. This is a documentation defect about which mechanism does the refusing, and the decision of whether to close container ports at the host layer as well — `DOCKER-USER` rules, or publishing to `127.0.0.1` and reaching containers another way — is a change of its own that this entry does not prejudge.
