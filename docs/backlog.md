...
What it did not supply is anything to serve, and `onboard-commerce-ops-to-staging` has moved that only partway. **Staging has the platform stack and an authorised application that has not yet deployed.** `deploy_apps` in `ansible/inventory/group_vars/staging.yml` now enumerates `commerce-ops` beside `platform`, so the host carries its `/opt` directory, its forced-command `authorized_keys` line and its sudoers rule — but nothing is deployed behind them, so Traefik there still routes nothing and still declares no router for ACME to answer for. Opening 80 and 443 today would still put a reverse proxy with no backend on the public internet, and would still issue no certificate.

**So the trigger is an application reaching staging, not a date** — and it is approaching rather than met. What is outstanding, in order: the deploy itself, which needs `provision-commerce-ops-database-in-the-shared-instance` and a staging deploy path in the application's own repository; then both firewall layers opened together and one DNS A record, which is this entry. Taking it before the deploy would still mean writing a dummy router to have something for ACME to answer for — a contrivance that rehearses the mechanism against a case production will never run.

**The tailnet is not a way around this entry, which is worth stating because it reads as though it should be.** Traefik routes on the `Host` header, so a request that arrives is routed whatever the hostname resolves to — but UFW's default-deny reaches `tailscale0` exactly as it reaches the public interface, and `ansible/roles/hardening` opens 80 and 443 only from `hardening_web_allowed_cidrs`, which is `[]` here. Its two tailnet exceptions are hard-coded for 22 and 3000. A tailnet peer's request to port 80 is dropped, and what remains is observing from the host itself over loopback, which UFW does not filter.

**One consequence of the wait, so it is not chased as a fault.** Once an application's router does exist on staging, Traefik begins a TLS-ALPN-01 issuance it cannot complete — Let's Encrypt answers that challenge by connecting inbound to 443, which is closed at both layers — and retries on its own schedule, logging failures until this entry lands. Expected, and harmless to the deploy.
...
- **What is reachable before DNS exists, which is less than it looks.** Traefik routes on the `Host` header, which invites the conclusion that an application on a host with no public hostname is testable over the tailnet with a `Host:` header. It is not: UFW's default-deny reaches `tailscale0` as much as the public interface, and `ansible/roles/hardening` opens 80 and 443 only from `hardening_web_allowed_cidrs` — `[]` on a stack with no web exposure — its two tailnet exceptions being hard-coded for 22 and 3000. What works is observing **from the host**, where loopback is unfiltered and Traefik publishes `80:80` and `443:443`: `curl -k --resolve '<hostname>:443:127.0.0.1' https://<hostname>/`, over `https` because the `web` entrypoint redirects unconditionally and with `-k` because no certificate can issue while 443 is closed (the resolver uses TLS-ALPN-01, which Let's Encrypt performs by connecting inbound). A browser therefore needs the ports opened and a DNS record — entry 17 — and nothing short of it. That distinction decides whether a given stack needs its web ports opened at all, and it was written down nowhere until `onboard-commerce-ops-to-staging` established it.
