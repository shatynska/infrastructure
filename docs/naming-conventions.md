...
| platform deploy key, **one per environment** | `~/.ssh/shatynska-platform-production` | `~/.ssh/fuperia-platform-production` |
| application deploy key, **one per application per environment** | `~/.ssh/shatynska-commerce-ops-staging` | `~/.ssh/fuperia-<app>-staging` |
...
**The two deploy keys carry the environment**, because what a deploy key authorises is an entry in `ansible/inventory/group_vars/<environment>.yml`: `deploy_apps` sits on the environment axis, so the file holding a key's public half — the platform's included — is an environment's file. A name spelling the stack would claim a narrower blast radius than the key has. The platform key's *stored* half is per stack all the same, in that stack's own GitHub Environment; the axis a name takes is the one its authorisation sits on, not the one its storage does.

**The converge key carries the stack, not the environment**, and it is the only per-stack artefact whose name lives on a workstation rather than in a namespace some service owns. It differs from the two above in the way that decides the axis: no `group_vars` entry stands between it and the host — its public half is appended to `root`'s `authorized_keys` on one stack's host by hand. That is what made it the last one to move: nothing reads the filename — the private half goes into a GitHub Environment secret and the public half into a host's `authorized_keys`, and neither carries it — so no check can hold this name and only the runbook states it. Two stacks made `-production` and `-staging` distinct by luck; a second tenant would have collided, which is the collision `HCLOUD_TOKEN_MAIN_PRODUCTION` carries four extra characters to avoid one namespace over.
