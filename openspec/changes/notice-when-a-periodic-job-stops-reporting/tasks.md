...
- [x] 2.1 Add a `report` job to `.github/workflows/drift.yml`: `needs:` every other
...
- [x] 2.2 The same job in `.github/workflows/pre-commit-autoupdate.yml`, slug
  `infrastructure-pre-commit-autoupdate`.
- [x] 2.3 Give each reporting step a comment naming what it is for and what it is
  *not*: that silence, not the ping, is what raises the alarm; that the vendor-side
  ...
  timer (Decision 7).
- [x] 2.4 Confirm the `drift.yml` reporter does not disturb the existing drift-issue
  ...
  (`design.md` Non-Goals).
- [x] 2.5 Before building either URL, the step SHALL fail with a message naming
...
- [x] 3.1 In `ansible/roles/image_prune/defaults/main.yml`, add
...
- [x] 3.2 Give the role the named-input assertion for
...
- [x] 3.3 Install a root-owned `0700` reporting script that sources
...
- [x] 3.4 Add `ExecStopPost=` to the `prune-host-images.service` unit — written
...
- [x] 3.5 The reporting change SHALL NOT cause the unit to run. Installing or
...
- [x] 3.6 Update **every existing** `image_prune` Molecule scenario — `default` and
...
- [x] 3.7 Record in `ansible/roles/image_prune/README.md` what the reporter is, the
...
- [x] 4.1 Dispatch the test author with the `.github/tests` row —
  ...
  to on a host.
- [x] 4.2 `.github/tests`: **every** workflow whose triggers include `schedule`
...
- [x] 4.3 `.github/tests`: the reporting job depends on every other job in its
...
- [x] 4.4 `.github/tests`: no reporting job declares an `environment:`, and the
  secret it reads is not one of the `PLATFORM_*` Environment secrets (Decision 6).
- [x] 4.5 `.github/tests`: the cancellation path reports nothing, and every ping
  URL a workflow carries — success and `/fail` alike — sets `create=1`
  (Decision 5).
- [x] 4.6 `.github/tests`: the slugs of **every reporter it discovers** — each
...
- [x] 4.7 `.github/tests`: every role that installs a `.timer` unit also installs
...
- [x] 4.8 Molecule, `image_prune`: after convergence the unit carries
  ...
  performs no prune.
- [x] 4.8a `.github/tests`: the reporting script's committed text passes the URL to
  ...
  obliges.
- [x] 4.8b `.github/tests`: no `image_prune` Molecule scenario leaves
...
- [x] 4.9 Molecule, `image_prune`, through the base-URL seam of 3.1 pointed at a
  ...
  with a sleep.
- [x] 4.10 Molecule, `image_prune`: converging with no ping key supplied fails
  naming that input, before the host is changed (3.2, Decision 13).
- [x] 4.11 The static suite must stay inside its stated constraints: no network
...
- [x] 5.1 `README.md`: extend the runbook section that covers re-enabling the
  ...
  check as evidence.
- [x] 5.2 `docs/bootstrap-a-new-host.md`: add `HEARTBEAT_PING_KEY` to the Appendix A
...
- [x] 5.3 `docs/bootstrap-a-new-host.md`: the "What to change for a company host"
...
- [x] 6.1 Provision the working tree before reading any verification result:
...
- [x] 6.2 Run `python3 -m unittest discover --start-directory .github/tests` from
...
- [x] 6.3 Run `pre-commit run --all-files` and confirm `ansible-lint`,
