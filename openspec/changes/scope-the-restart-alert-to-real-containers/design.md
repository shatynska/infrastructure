...
Asserting that the `expr` string contains `name!=""` twice would pass on `container_start_time_seconds{name!=""}` written twice and on one written twice with the other left bare, and would fail on a reformat that put a space inside the braces. The assertion instead loads `platform/docker-compose.yml`, parses the `prometheus_rules` entry's `content` as YAML, locates every rule reading a `container_*` series, extracts every metric selector in its `expr`, and requires each to carry a `name` matcher that excludes the empty string. A half-applied selector is then a failure, and a reformat is not.

Locating by the metrics a rule reads rather than by its `alert` name is the test author's decision, and it is the better one: a rename that carried the defect along with it would pass a name-based read silently, and the requirement binds "an alert rule whose subject is a container" rather than one particular rule. The obvious risk of locating by metric is going vacuous — rename the metrics and the rule set is empty and everything passes — and the module closes it with non-vacuity guards that turn an empty set into a red suite naming the absence, plus a separate assertion pinning both metric names individually.

One latent consequence, recorded so the next author is not surprised by it: the check flags *every* selector of a container-subject rule, including one that could carry no container name. A future rule joining `container_oom_events_total{name!=""}` against `up{job="cadvisor"}` would be reported as an offender on the `up` selector. That is faithful to the requirement's literal text and harmless today, since the one container-subject rule reads container metrics on both sides.
