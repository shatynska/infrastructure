...
- [ ] 2.4 Confirm `HCLOUD_TOKEN` is not an organization-wide or admin-level credential — it SHALL be scoped to only the dedicated prod Hetzner project, at the minimum permission level (Read Only / Read & Write) each secret needs. `TF_API_TOKEN` is exempt from this check by design: it's a deliberately unsplit, org-scope-unavoidable token on this HCP Terraform free tier (see design.md Decision 9) — `HCLOUD_TOKEN`'s split is what this requirement is actually protecting. **Not yet actionable:** confirmed via `gh secret list` / the environments API that no repository or environment secrets exist yet at all — blocked on tasks 1.3/1.5/2.2/2.3/2.8 actually creating and storing the real tokens.
