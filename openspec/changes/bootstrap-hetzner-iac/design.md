...
**9. Both `TF_API_TOKEN` and `HCLOUD_TOKEN` are static tokens, split by privilege.**
...
Given that, `TF_API_TOKEN` is treated like `HCLOUD_TOKEN`: split by privilege using HCP Terraform's per-team workspace permission levels (Plan vs. Write), each token scoped to a team with access to only the `infrastructure-prod` workspace — never an organization-wide or admin token. A repository-scoped `TF_API_TOKEN` (Plan permission: read state, acquire a read lock) shadows to a `production` Environment-scoped `TF_API_TOKEN` (Write permission) for the gated apply job, via the same environment-secret-shadowing mechanism as `HCLOUD_TOKEN`. This keeps the halved-static-secret goal in spirit — no *admin*-level long-lived token exists — even though the original goal of eliminating a static HCP credential entirely does not hold.

An alternative worth one sentence: dropping HCP Terraform entirely in favour of an S3 backend on Hetzner Object Storage — Terraform 1.11+ `use_lockfile` provides native state locking with no DynamoDB equivalent required — would keep state, compute, and credentials inside a single vendor. It was considered and not chosen for this pass (see task 1.3's implementation notes); HCP is kept for its state versioning and rollback and to avoid bootstrapping a bucket before Terraform can run, but the option is recorded rather than left unexamined, and is worth revisiting now that the "no static HCP token" advantage no longer applies.
...
- HCP Terraform workspace permissions could deny state writes to the operator's own user while granting them Plan-level access, the same permission level CI's own repository-scoped `TF_API_TOKEN` now uses (Decision 9). Whether that cleanly permits local `plan` — which reads state and takes a lock — while blocking local `apply` depends on how HCP's permission levels map onto those operations; verify during implementation before relying on it, in the manner of Decision 7's `delete_protection` caveat.
...
1. Create the HCP Terraform organization/workspace (`infrastructure-prod`, CLI-driven, **Execution Mode = Local**), create two teams scoped to only that workspace (Plan permission, Write permission) and generate a team API token from each for `TF_API_TOKEN`, and create the Hetzner Cloud project plus its Read Only and Read & Write tokens.
