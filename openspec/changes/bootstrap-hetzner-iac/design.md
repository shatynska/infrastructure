...
- **`delete_protection = var.delete_protection` on `hcloud_server`** (with `rebuild_protection` set to match, as the provider requires). This is parameterizable per environment, and unlike `prevent_destroy` it sets a server-side lock that also blocks deletion from the Hetzner console/API — a hole nothing else in this design covers. **Verified finding (task 4.6):** the `hetznercloud/hcloud` provider does not reliably block or fail fast on `terraform destroy`/`terraform apply` (replace) when `delete_protection = true`. Multiple open upstream bug reports describe divergent bad behavior across versions — the destroy either hangs indefinitely waiting on an API call the provider never stops retrying (hetznercloud/terraform-provider-hcloud#1014), or the provider does not block the operation at all, silently destroying a "protected" resource (hetznercloud/terraform-provider-hcloud#519). Neither failure mode is a safe stop — one wedges the run, the other defeats the protection outright. Do not treat `delete_protection` as the Terraform-side guard for either destroy or replace; the CI destroy-policy gate (Decision/task 6.4), which inspects the plan before anything is applied, is the actual guard, and `delete_protection` is kept only for its independent value of blocking deletion from the Hetzner console/API outside of Terraform entirely.
...
**9. `HCLOUD_TOKEN` is split by privilege; `TF_API_TOKEN` is a single unsplit token, an accepted risk.**
The original plan assumed HCP Terraform's GitHub OIDC workload identity could eliminate `TF_API_TOKEN` as a long-lived repository secret, the same way OIDC eliminates a static credential for a cloud provider. **Verified during implementation, and wrong:** HCP Terraform's dynamic-credentials/workload-identity feature authenticates a *provider* (AWS, Azure, GCP, Kubernetes, Vault, the HCP provider) during a run that HCP Terraform itself executes remotely. It has no mechanism for authenticating the Terraform CLI's own backend/state access when the CLI runs externally to HCP Terraform — which is exactly this repository's situation, since Decision 1 chose CLI-driven, Local execution specifically to keep the whole pipeline in GitHub Actions. HCP Terraform's own documentation describes exactly one supported mechanism for that case: a static API token, presented via `TF_TOKEN_app_terraform_io` or `setup-terraform`'s `cli_config_credentials_token` input.

The first fix attempt proposed splitting `TF_API_TOKEN` by privilege the same way as `HCLOUD_TOKEN`, using HCP Terraform's per-team Plan/Write workspace permission levels. **Also wrong, also caught during implementation:** Teams — and therefore team API tokens — are a paid-tier HCP Terraform feature (Essentials and above). On the free tier this organization is on, every token (user or organization) has full access to every workspace in the org; there is no way to mint a read-only credential.

Given that, `TF_API_TOKEN` is a single token (an Organization API Token, or a dedicated user's token — either is equally unscoped on this tier, so pick whichever is easier to rotate), stored as an identical value in both the repository-scoped and `production` Environment-scoped `TF_API_TOKEN` secrets. This is a deliberately accepted risk, not an oversight: a compromised PR-time or drift job holding this token could read or corrupt Terraform *state* (e.g. remove a resource from tracking, forcing a future apply to misbehave), but — critically — it still cannot mutate real Hetzner infrastructure, because it lacks `HCLOUD_TOKEN`'s write-capable value, which remains genuinely split and is the actual load-bearing security boundary in this design. The two secrets are kept as separate GitHub secrets (not one shared name) purely so the environment-secret-shadowing mechanism stays wired up in the workflows: if this org is ever upgraded to a paid tier, only the two token *values* need to change — no workflow code does.

An alternative worth one sentence: dropping HCP Terraform entirely in favour of an S3 backend on Hetzner Object Storage — Terraform 1.11+ `use_lockfile` provides native state locking with no DynamoDB equivalent required, and a hand-written bucket policy could achieve a genuine read/write credential split. It was considered and not chosen for this pass (see task 1.3's implementation notes) — HCP is kept for its state versioning and rollback, and to avoid taking on a bucket-policy-authoring exercise for what is, in the end, still a single-workspace repository. Worth revisiting if this repository ever needs the split enforced rather than merely accepted as a risk.
...
The `HCLOUD_TOKEN` split from Decision 9 already mints exactly the right two credentials; it simply never said where a human's copy comes from. This decision extends that split from "CI jobs" to "CI jobs and workstations": the **Read & Write** token exists in exactly one place — the `production` GitHub Environment secret — and is never exported into a shell, written to a dotfile or `direnv` file, or stored in a credential helper. Local work uses the **Read Only** token, which is sufficient for `terraform plan` and refresh, and which makes a local `terraform apply` fail at the Hetzner API rather than by convention. This is the load-bearing control for the workstation boundary; `TF_API_TOKEN` (unsplit, per Decision 9) does not contribute to it — a workstation with only the read-only Hetzner token still cannot mutate infrastructure even if it also holds the single HCP Terraform token.

One supporting layer that turned out not to be available, and one that is:
- HCP Terraform workspace permissions denying state writes to the operator's own user while granting read/plan access would have been a second layer here — but per Decision 9, this organization's free tier has no per-user or per-team permission granularity at all, so this layer does not exist on this plan. Revisit only if the org is ever upgraded.
- The prohibition is written where it will actually be read: the README runbook for a human, and a repository-root `AGENTS.md` for an agent. Both files already exist, added by a separate, already-archived `project-foundation` change — `AGENTS.md` already states this exact prohibition under "Production changes never bypass the pipeline," and the README already documents local setup and the drift-workflow re-enable runbook. Tasks 3.6 and 3.7 are scoped as verification/extension against that existing content, not as first-time authorship.
...
1. Create the HCP Terraform organization/workspace (`infrastructure-prod`, CLI-driven, **Execution Mode = Local**), generate a single Organization (or user) API token for `TF_API_TOKEN` — this org's free tier has no team-scoped tokens, see Decision 9 — and create the Hetzner Cloud project plus its Read Only and Read & Write tokens.
