...
- **controller-resolved inclusion of variables**: `vars_files`, `include_vars`, and `include_role`'s `vars_from`;
...
- **a path argument on a task that delegates to the controller**, which is where a delegated `slurp`, `fetch`, `shell` or `command` belongs. An undelegated `slurp` reads the managed node and is out of scope by construction: `slurp` has no `remote_src` option at all, so "unless `remote_src`" is not the discriminator for it — delegation is;
- a path in a scenario's `molecule.yml` provisioner `env:`, which is neither a task nor a play.
...
**The corrected result.** Thirteen entries cover every read of a repository file from the controller across this repository's seventeen authored scenarios — twelve individual sites, plus one declaration repeated in every scenario. None of them reads any path this change excludes:
...
`dorny/paths-filter` matches with picomatch and supports `!` patterns within a filter, with the documented constraint that a file must first be matched by a positive pattern before a negation can exclude it — which is why the positive `ansible/**` stays at the head of the list.

`.github/tests` can assert **what the filter declares**: that the `ansible/**` positive is present, that each of the four negations is present, and that no negation was added that this design did not establish. It cannot assert what the action *does* with those patterns, because reproducing picomatch would need a dependency the suite is forbidden from taking, and running the action would need a network call it is forbidden from making. Those constraints are themselves asserted by tests in that module.
