...
**What option 3 trades.** An earlier draft of this decision claimed it left an order-of-**merge** hazard: that change is at `ship`, so if it archived before this change merged, the specification would oblige `terraform/stacks/` over a tree still holding `terraform/environments/`. **That is not reachable, and the reason is worth writing down rather than leaving as a worry someone re-derives.** The sweep lives only on this branch — `origin/main`'s copy of that delta still carries four occurrences of the old path — so an archive taken before this change merges applies the *unswept* text, and one taken after applies the swept text to a tree that has already moved. The delta and the directory travel in the same pull request, and both orders are consistent.

What is real is narrower and is a constraint on this change rather than on that one: **this branch must not merge in its plan-and-tests state.** That would put swept delta text on the trunk over a tree that has not moved, and nothing detects it — `.github/tests` reads `openspec/` only for the citation-form check and never compares a cited path against the tree, and `openspec validate` checks structure rather than paths. Recorded as tasks.md 7.3.
