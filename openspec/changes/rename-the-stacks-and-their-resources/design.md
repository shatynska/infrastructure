...
**`strict: true` reaches `keyed_groups`, and a host missing the key fails the WHOLE parse rather than being left ungrouped. This decision said the opposite when it was written, and the correction was bought by a real run.** The staging converge of this change lost a three-second race with this change's own Terraform apply — it read Hetzner at 05:38:04 and the apply added the label at 05:38:07 — and failed with *"Could not generate group for host staging-server from hcloud_labels.tenant entry: object of type 'dict' has no attribute 'tenant'"*. `Constructable._add_host_to_keyed_groups` raises `AnsibleParserError` under `strict`, and `ansible.cfg`'s `any_unparsed_is_failed` fails the run.

The claim came from a comment already in the inventory sources, which this change repeated without testing the plugin it applies to; the probe that appeared to confirm it used `ansible.builtin.constructed` over a static host var, which does not reach the same templating path. **A probe that reaches the wrong code path is worse than no probe, because it is reported as a measurement.**

The behaviour is fail-closed and is kept: a host carrying one axis label and not the other is half-provisioned, and an inventory that resolved it into a group set nobody declared is how a converge reaches the wrong host set. What it costs is stated in Decision 10, which prescribed the right ordering for a weaker reason than the real one.
...
**Staging races, and this decision called that tolerable on a reason that turned out to be incomplete.** The reasoning was that staging's environment value does not change, so only the server rename reaches it. That is true and it is not the whole of it: the `tenant` label is added to *every* stack, and a host carrying `environment` but not yet `tenant` fails the inventory parse outright (Decision 5). **On the real merge staging's converge lost that race by three seconds** — it read Hetzner at 05:38:04 and the apply landed the label at 05:38:07 — and failed before the first play with the plugin's own diagnostic. Re-running it after the apply succeeded.

So the ordering below was right and its stated justification was weaker than the real one. It cost a re-run and nothing else, because the failure is loud and lands before any task acts on a host — but the general form is worth carrying forward: **any change that adds a label every stack's inventory keys on has a merge-to-apply window in which no converge can resolve**, and a stack whose apply needs no reviewer will enter that window unattended.
