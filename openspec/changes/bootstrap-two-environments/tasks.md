...
- [x] 1.1 Re-read the three single-environment mechanisms against the tree and record
...
- [x] 1.2 Confirm what the pipeline genuinely does at two environments by reading the
  ...
  **The carve-out, because task 2.8 needs it:** three paths were recorded there as
  ...
  observed behaviour.
- [x] 1.3 Establish what a first push to a new repository actually does, by reading
...
- [x] 2.1 Stage 0: the document's "What you will have at the end" list — the four
...
- [x] 2.1a Delete the block at stage 3's close beginning "**Adding a *second*
...
- [x] 2.1b Stage 0.2 and 0.3: the tools table and the SSH-key table. **0.3 carries a
  ...
  committed `tfvars`.
- [x] 2.2 Stage 1: two Hetzner projects and four tokens. Present one-project-or-two as
...
- [x] 2.3 Stage 1.3: the sizing table gains a second column of values, prod first,
...
- [x] 2.4 Stage 2: two HCP workspaces, `infrastructure-<environment>`, both CLI-driven,
...
- [x] 2.5 Stage 3: two GitHub Environments and the full secret set for both, including
...
- [x] 2.6 Stage 3: the reviewer asymmetry — `production` requires one, the lower
...
- [x] 2.7b Stage 3.1 and stage 4.2: the push sequence that actually applies
...
- [x] 2.7c **Keep** stage 3.1's closing warning that the push must find its secrets in
  ...
  not two.
- [x] 2.8 Stage 4: one apply run covering two environments — prod waiting for its
...
- [x] 2.9 Stage 4: the local `plan` step and the `.envrc` guidance become
...
- [x] 2.10 Stage 4.3 and 4.4: two addresses, one destination. `terraform output` runs
...
- [x] 2.11 Sweep every stage's **"Secrets created in this stage"** and **"Check"**
...
- [x] 3.1 Open stage 5 with what the second host cannot have yet: no Ansible run can
...
- [x] 3.2 State what the second host therefore *is* at the end of the document — a
...
- [x] 3.3 Appendix A: the secret inventory covers both environments, with each row
...
- [x] 3.4 Appendix C: fold the two-environment material into the company notes,
...
- [x] 3.5 Appendix B (rebuilding an existing host): state which host it covers. After
...
- [x] 4.1 Add a `docs/change-queue.md` entry for making `platform-deploy.yml`
...
- [x] 4.2 Verify entry 50 already covers the two Ansible mechanisms, and extend it only
...
- [x] 5.1 Run `python3 -m unittest discover --start-directory .github/tests` from the
...
- [x] 5.2 Run `openspec validate --all` and `openspec validate --archived`, and confirm
...
- [x] 5.3 Read the rewritten stages 0 to 4 against the repository as committed,
