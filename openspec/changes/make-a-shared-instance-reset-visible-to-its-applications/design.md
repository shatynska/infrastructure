...
- **Binding a key to one host.** A probe key is declared in `ansible/inventory/group_vars/<environment>.yml` beside its deploy key and inherits exactly the same over-authorisation a second tenant would expose. `bound-a-deploy-key-to-one-host-when-an-environment-holds-two-stacks` is that change and this one must not pre-empt its layout decision.
...
**The handover between the two is standard input and never an argument**, for the same reason the SSH layer's is: `sudo`'s command line is in the host's process listing. `deploy-probe` validates the object and forwards it to `app-probe` unchanged, as the same JSON — one format across both hops rather than a second internal encoding for a later reader to discover by reading both scripts.
...
- **A second `authorized_keys` entry per application doubles what a `group_vars` typo can mis-authorise** → The probe key grants strictly less than the deploy key beside it; a key mixed up between the two fields fails closed, since each entry's forced command is what it can run.
