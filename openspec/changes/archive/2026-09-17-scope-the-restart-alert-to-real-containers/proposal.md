...
- Deployment reaches both stacks through the existing `Platform Deploy` workflow on merge to `main`, and reaches them only because the `prometheus` service's `platform.config-checksum` label moves with the edit. An inline config's content is outside the digest Compose compares, so the label is what makes the deploy recreate the container; without moving it the deploy would report success and change nothing.
