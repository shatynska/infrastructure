...
So the exemption is one path: the new module itself, which has to state retired names in order to assert their absence. The other six modules are swept and corrected like any other file.

**The two document exemptions cost more than this decision first credited, and the measurement belongs here rather than in a footnote.** `docs/deferred-work.md` cites **17** live requirement names and `docs/change-queue.md` cites **13** — a citation surface of the same order as the `.github/tests/` density this decision spends its length refusing to exempt. The change-queue one expires at archive. The deferred-work one does not, so the next rename lands there unread. The requirement admits only whole-path exemptions, so nothing narrower is available to this change; closing it would mean moving the one historical sentence somewhere the sweep reads, or giving the requirement a per-occurrence exemption form. Neither is this change's, and both are now stated at the exemption itself.
