...
**The refusal has a limit, and it is worth naming rather than discovering.** It refuses a construction it *parsed a path out of* and did not recognise. It cannot refuse one it never saw — a spelling the matcher does not reach produces no read at all, and silence looks exactly like absence. Code review found four such spellings after the first implementation was green: the `query()` and `q()` aliases for `lookup()`; a `delegate_to` whose value is quoted or followed by a comment; a repository path sitting inside a `${VAR:-default}` fallback, which the scalar rule reduced to nothing before testing it; and a play named `.yaml` rather than `.yml`. All four are fixed, and every one of them was a *silent* miss rather than a refusal — which is the point. So the polarity argument of Decision 2 covers unrecognised constructions and not unparsed ones, and closing that second gap is a matter of the matcher's breadth, checked by probing spellings deliberately rather than by trusting the refusal to catch them.
