...
- **Every fixture derives its device from that base plus a fixed offset** rather than from a hardcoded literal, in all **ten** plays that name one — the `prepare.yml`, `converge.yml` and `verify.yml` of each of the three single-device scenarios, and `multiple-devices-discoverable/prepare.yml`, which two scenarios share. Offsets 0 through 6 are used; 0 through 4 preserve today's scenario-to-minor ordering, and 5 and 6 deliberately do not, because the two `multiple-devices` arms take separate pairs rather than the one pair they shared as literals.
...
**Ansible.** `ansible/scripts/run-molecule` gains the derivation and a second print flag. **Ten** fixture files under `ansible/roles/platform_data_volume/molecule/` change: the ten plays enumerated above. `multiple-devices-reverse-order` imports its sibling's `prepare.yml` and has no play of its own to edit.

**No `molecule.yml` changes, and the plan said three would.** Written into the plan on the strength of a `grep` for `losetup`, which matched three scenario definitions; reading them showed every match is in a comment justifying `privileged: true` — `mknod`/`losetup`/`mount` needing `CAP_SYS_ADMIN` — and none names a minor or explains the hardcoding. No scenario definition mentions the scheme at all. The prose describing the hardcoded minor lives entirely in the fixture plays' own headers, which change with the plays.
