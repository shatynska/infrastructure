...
**The run SHALL report on every exit path, and a run that was abandoned SHALL fail.** A run that completed SHALL report how many images it considered, how many it removed, and how many removals the runtime refused. All three counts SHALL be deduplicated by image identity, so that an image carrying several tags counts once however many removals it took. The considered count SHALL be the number of distinct identities the run enumerated. The removed count SHALL be the number of **those** identities for which one of this run's own removal invocations reported the image deleted — anchored to the invocation, not to the host's state at the end of the run. An image a concurrent actor removed is therefore not counted as this run's work, and an image this run deleted is still counted where a concurrent deploy has since re-pulled it. A tag skipped because it no longer named the image it was selected as, and a tag dropped from an image that survives under another, each leave it unchanged. A run abandoned early SHALL instead report which condition ended it — an unresolvable or malformed reference set, an enumeration the host does not yet carry, an enumeration naming no application, an empty keep set, or a failure to enumerate local images — and SHALL be distinguishable from a completed run that found nothing to remove. Where the enumeration is unavailable because the host has not yet been configured with one, the report SHALL say so distinguishably from an enumeration that is present and names no application: the two have different remedies, and a report that conflates them sends an operator to the inventory when the host merely needs a converge. Unlike deploy-time reclamation, which SHALL NOT change the outcome of a deploy that already succeeded, this unit has no caller to damage and SHALL therefore exit non-zero on an abandoned run, so that the host records a failed unit rather than a silent one.

**The refused count SHALL be the number of those identities for which one of this run's own removal invocations was rejected by the container runtime**, and SHALL likewise be anchored to the invocation rather than to what survives on the host. It exists because the shortfall between the considered count and the removed count carries no signal on its own: the considered count is every identity the host holds rather than the set the run offered for removal, so a run whose keep set was defective and whose removals were therefore blocked is indistinguishable, by those two counts alone, from a healthy run over a host where everything is referenced. An identity both deleted and rejected within one run SHALL count as removed and not as refused, so that the removed and refused counts together never exceed the considered count.

**A removal that the runtime performed without deleting the image SHALL NOT be counted as refused.** Dropping one tag of an image that survives under another is the ordinary shape of removing a multiply-tagged image and is not a refusal; a count that included it would read non-zero on healthy runs over ordinary hosts, which is the defect this count exists to remove rather than to relocate. What distinguishes the two is whether the runtime rejected the invocation, and that SHALL be what the count is read from — not whether the image is still present afterwards, which is true of both.

**Each refused removal SHALL additionally be reported individually, naming the identity and the reason the runtime gave.** A count establishes that something was refused and not which image or why, and the causes have different remedies: an image a container holds means the keep set is defective and the runtime's refusal is what stood between it and a live image, while an image carrying no tag and more than one reference cannot be removed by this run at all and will be offered and refused on every subsequent run. A report conflating those sends an operator to look for a defect that is not there, on the same reasoning this requirement already gives for keeping an absent enumeration distinguishable from an empty one.

**A refused removal SHALL NOT change the run's outcome.** It SHALL remain a normal outcome per this requirement's removal clause, SHALL NOT be retried, and SHALL NOT cause the run to exit non-zero: the refusal is the backstop working, and a unit recorded as failed because its backstop worked would report the opposite of what happened.
...
- **THEN** it SHALL report how many images it considered, how many it removed and how many removals the runtime refused, each counted over distinct image identities, and SHALL exit zero

#### Scenario: A run whose removals were all performed reports no refusals
- **WHEN** a scheduled run completes and the runtime rejected none of its removal invocations
- **THEN** the refused count it reports SHALL be zero, so that a run over a correctly protected host is distinguishable from one whose removals were blocked

#### Scenario: A refused removal is counted and named
- **WHEN** a scheduled run offers an image for removal and the container runtime rejects the removal
- **THEN** the run SHALL count that identity as refused, SHALL report the identity and the reason the runtime gave, and SHALL still exit zero, that image being unchanged on the host

#### Scenario: A refusal is distinguished from a removal that deleted nothing
- **WHEN** a scheduled run drops one tag of an image the runtime keeps, the invocation succeeding without deleting the image
- **THEN** that identity SHALL NOT be counted as refused, the runtime having rejected nothing
