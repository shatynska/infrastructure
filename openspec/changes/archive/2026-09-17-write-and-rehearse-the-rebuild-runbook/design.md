...
What the change does owe is the missing half of the register — and the shape of that gap is not what this design said for three review rounds. **It was recorded here, and through two rounds of review, that no committed file gave the per-stack Alertmanager checks a period and a grace.** That was derived from Appendix A's "Not in this table" paragraph and from `platform/README.md`'s rule of thumb, and it is false: §7.1 states both, five minutes each, beside the instruction to create the check. The claim was never checked against §7.1, and the reviewer accepted it on this document's word. It was caught by the derived tests — the assertion that every check a host reports to has its settings recorded failed, and reading what it failed on is what sent someone to §7.1.

So the real defect is smaller and of a different kind: the values exist, and the register `README.md` calls their single home does not hold them. That is the single-source claim being false rather than a value being absent, and it is fixed by moving what §7.1 states into Appendix A's new block and leaving §7.1 the creation step with a citation. **No observer read is needed to write the block**, which is what dissolves an ordering conflict this design would otherwise have carried: the derived module asserts the register in pull request 1, while the rows had been scheduled for pull request 4 on the belief that only the rehearsal could learn them.

The rehearsal still has work here, and it is now the right work: it reads the two checks at the observer and confirms the settings there match what the register claims. A register nobody has compared against the observer is a claim, not a record — and the rebuild is the one occasion that comparison is owed anyway.
