...
**That rule reaches the root-side read too, and the line it draws there is narrower than on the client path.** An earlier draft answered `unreachable` for every failure of the `docker exec`, which asserts something about the instance from what may be a failure of the probe's own machinery; this change's code review named it. But most of what the runtime can say *there* genuinely is about the instance — `Container … is not running` and `No such container` are precisely the stopped and absent cases the requirement names, and both must stay `unreachable`. Only a runtime that cannot be asked at all — no binary, no daemon, no permission to reach it — is a refusal. On the client path the list is wider, because there the runtime is being asked to start a container of our own and its failures say nothing about the instance either way.
...
**Neither value may carry a NUL, and that is part of the transport rather than a nicety.** The two values are handed from the parser to the probe NUL-separated, so a password carrying one emits three fields where the reader expects two: the probe takes the password's head as the password and its tail as the *table name*, which never passed the shape check, while the real name is discarded unread. JSON's own escape for a NUL decodes to one, so this is reachable from the wire. This change's code review found it, and it was confirmed against the parser before being fixed. Nothing executes either way — the smuggled name still reaches SQL only as a quoted `to_regclass` argument — but the belt below would have been bypassed and the probe would have answered `empty` about a name it was supposed to refuse.

**The handover itself touches no file.** The parser's output is read through a process substitution rather than a temporary file: a command substitution would drop the NUL framing, and a file would put the password on the host's filesystem for the life of the probe — root-only and trap-removed, but a fourth landing that none of the disclosure checks sample and one a SIGKILL would leave behind.
