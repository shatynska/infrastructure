...
Lets the prod stack provision a persistent Hetzner Cloud volume, attached to the prod server, that can be toggled out of management without losing its declared configuration — mirroring how the prod server itself is toggled.

## MODIFIED Requirements

### Requirement: Conditional Prod Volume Creation
The prod stack SHALL expose a boolean variable that controls whether the volume module creates any resources, independent of the rest of the volume's configuration (size, name).
...
- **WHEN** the prod stack's volume-enabled variable is `true` and the prod server is also enabled
...
- **WHEN** the prod stack's volume-enabled variable is `false`
