...
**A non-zero exit is not by itself a rejection, and one case has to be excluded by name.** `docker image rm` also exits non-zero when the image is already **gone** — which happens here for a real reason, not a hypothetical one: a concurrent `app-deploy` reclaims images too, the weekly window will occasionally overlap a deploy, and the by-identity branch has no pre-removal re-check to narrow that window the way the by-tag branch does. The runtime rejected nothing in that case; it answered that there is nothing to reject. Counting it would send an operator hunting a keep-set defect that does not exist, and it would contradict the position the requirement already takes one clause earlier, where an image a concurrent actor removed is explicitly not counted as this run's work. So a `no such image` message excludes the increment, using the same `grep` on the same slot as the keep-set resolution — which draws the "could not ask" against "answered nothing" line in exactly the same place, for the same reason.

**What that exclusion does not reach, stated rather than implied.** A runtime that stops responding part-way through the removal loop makes every remaining invocation fail for a reason that is not `no such image`, so each remaining candidate is counted and named as refused and the run still reports a completed line and exits zero. That is not a regression — before this change the same run reported `removed 0` and said nothing at all — and each line carries the runtime's own error, so what happened is legible. But it is a case where the count means "this run could not remove it" rather than "the runtime refused it", and no black-box scenario can arrange it: wedging the daemon breaks the same daemon the scenario's fixtures need, which is why the existing scenario already declines to assert the duration bound. It is held by review and recorded here rather than left to be discovered.

**And it has a consequence at the observer, which is the sharp end of it.** The run exits zero, so `ExecStopPost=` sees `SERVICE_RESULT=success` and pings the **success** endpoint rather than `/fail`. A prune whose runtime died half-way therefore reports itself healthy to the external dead-man's-switch — the only thing watching this unit — and the journal is the only place the truth exists. That is not made worse by this change, since before it the same run reported `removed 0` and said nothing at all; but it is the state an operator would be most surprised by, so the README gives its signature: `refused` large and close to `considered`, with an **identical** message on every named line. A genuine refusal is one or two lines carrying different messages. It is also a sharper motivating case for the backlog entry this change opens on making a scheduled unit's own output alertable than the one that entry was written around.

The alternatives were considered and are both worse. Excluding the connection error alongside `no such image` recreates exactly the signal-free report this change exists to end — `refused 0`, exit zero, remaining candidates silently unprocessed. Exiting non-zero would add an abandon condition to a list the requirement enumerates exactly, and would be mechanically wrong besides: the requirement says an abandoned run removes nothing, which is false after this one has already deleted some. Either would be a specification change, and neither belongs in a change whose whole claim is that removal behaviour and the exit status are untouched.
