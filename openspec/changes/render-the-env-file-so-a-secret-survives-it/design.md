...
**What the first real firing showed, recorded because it corrected this decision's own wording.** On 2026-09-16 the deploy of the merge commit (run `35057453236`) named `GRAFANA_ADMIN_PASSWORD` on **both** stacks. Nothing had been corrupted: no container was recreated on either host — every service reported `Running`, not `Recreated` — and that value is interpolated into Grafana's `environment:`, so a changed parsed form would have changed the service definition and forced a replacement. The password therefore carries one of the characters whose position here does not corrupt: the backslash margin, a `#` not preceded by a space, or a mid-value apostrophe.

The detection behaved exactly as designed. **The message did not**: it said the values "would have altered" and "needs rotating", which is the narrow claim, while what the step tests is presence — deliberately a superset. So its first firing instructed an unnecessary rotation of a production credential, which is the failure a report like this exists to avoid. The wording now states what it detects and names the test that settles the rest: a value whose parsed form changed also changed its service definition, so that deploy recreated that service's container. One exception, which the message carries: a value interpolated into an embedded configuration replaces no container (Decision 8), so that test cannot see it.
