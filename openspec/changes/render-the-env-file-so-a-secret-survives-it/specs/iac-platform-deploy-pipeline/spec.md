...
**A value the rendering cannot represent SHALL NOT be written as something else.** The failure this requirement exists to prevent is silent: a corrupted or emptied credential starts its service, satisfies the deploy's wait for health, and leaves the run green, which is the same state *An Incomplete Per-Stack Secret Set Is Reported by Name* in this capability forbids for an absent secret. A rendering that converts an unrepresentable value into an empty one therefore SHALL NOT be adopted on the grounds that it preserves more values than the rendering it replaces.
