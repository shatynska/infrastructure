...
|---|---|---|---|
| 1 | Create the two new repository secrets | Any time before step 3 | The pull request's plan jobs read `secrets[<the name pipeline.yml declares>]`; an absent secret resolves to the empty string and the plan fails on authentication |
| 2 | The derived tests, the implementation commit, the documents, **and the whole of the offline verification, the tree-wide sweep and the code review** | Before step 3, all of it | Every one of them is offline. Running them after the rename would hold the window open for the length of a fix round, which is the cost step 3's placement exists to avoid |
| 3 | **Rename both HCP workspaces, then plan locally, then push** | **Immediately before the push** | `terraform init` *creates* an absent workspace, so a `versions.tf` pushed ahead of the rename takes the name the rename needs. The local plan is the only check that can run between the two |
| 4 | Merge, read staging first, approve production | — | — |
| 5 | Delete the two old repository secrets; delete any workspace created inside the window | After the merge | Nothing reads them once the merged `pipeline.yml` names the new ones |
| 6 | Rename both Hetzner projects | After the merge, before the confirmation | Cosmetic; and decision 7's observation has to follow it |

**A step 0 and a step between 3 and 4 were both removed when the GitHub Environment rename was deferred** — the probe that cleared the name, and the rename itself. Decision 4 records why.

**Step 3 is late, and an earlier draft of this document had it first.** The reasoning that put it first was that its window should open as early as possible so the pull request closes it immediately — which is the correct instinct and the wrong conclusion, because under this task list the pull request does not follow immediately. Between an HCP rename and a push sit the implementation, an independent test-author dispatch, the document edits, the offline verification and a code review: plausibly days, spanning at least one nightly drift run. The window is bounded by an operator instruction and by nothing else, so the only thing that actually shortens it is doing the rename later. **The window is now `rename → local plan → push → review → merge`**, which is the shortest the mechanism permits: the local plan needs both the renamed workspace and the flipped `versions.tf`, so the two cannot be separated, and the pull request cannot precede the rename because its own plan jobs would create the workspaces.

**What makes step 3 safe to defer is that nothing local reaches HCP before the push.** `pre-commit`'s `terraform_validate` hook runs `terraform init -backend=false` — hardcoded in `_common.sh` at lines 553 and 559 of `pre-commit-terraform` v1.109.1, the revision `.pre-commit-config.yaml` pins, with the only injectable argument being `-no-color`. `terraform_fmt` and `terraform_tflint` read no backend. So a commit made with the flipped `versions.tf` and the old workspace names still in HCP touches nothing remote. That precondition is checked rather than assumed, and it is version-dated because a hook revision bump could change it.

**There was a second window and there is not any more.** The GitHub Environment rename had to sit between approval and merge, because while `main` still declares `github_environment: production` and `platform-deploy.yml` still says `environment: production`, renaming that Environment makes any job reaching it **auto-create an empty one, with no protection rules** — so a production apply, converge or platform deploy in that window would run without the reviewer that is the entire point of the gate. Deferring the rename (decision 4) closed that window without opening it. It is described here rather than deleted because `docs/change-queue.md` entry 75 inherits it, and because it is the more dangerous of the two: the HCP window fails loudly and leaves litter, this one fails silently and leaves a production job ungated.
...
**The standing instruction for the remaining window is the operator's to hold: nothing else merges to `main` between step 3 and step 4.** A Dependabot pull request is safe — a pull request plans, it does not apply — but not free; see decision 2.
...
## Decision 3: Renaming rather than re-creating the GitHub Environments — and the rename does not exist
...
**This rested on a claim this repository cannot test: that GitHub offers an Environment rename preserving secrets and protection rules.** It was asserted by the handoff and by nothing else here, so it was probed rather than assumed — decision 4.

**The probe returned no, and this decision is therefore moot: there is no rename to prefer.** The choice it framed was between a cheap rename and an expensive re-creation, and only the expensive one exists. That turns the twenty-one-secret cost from a rejected alternative into the entry price, which is a different decision than the one made here — and one belonging to the operator, who took it: the Environment rename is deferred to `docs/change-queue.md` entry 75 and this change ships the other three renames. **What is worth keeping from this decision is its reasoning, because entry 75 inherits it unchanged**: the three unrecoverable credentials, the host-side steps each rotation needs, and the fact that a migration depending on a value nobody can produce fails at the worst moment.

## Decision 4: The probe ran, and it is the reason this change ships three renames instead of four

An Environment named `main-production` already exists: created 2026-09-12T06:11:52Z, no secrets, no protection rules, no deployments, and no workflow run at that minute — so it was created by hand, four minutes before `PLATFORM_DEPLOY_HOST` was rewritten, during entry 62's out-of-band steps. The plan was to rename it to `rename-probe` and delete it under the new name: free, and establishing at a cost of nothing that a rename control exists in this interface at all, which decision 3 depends on and no file here can assert.

**It does not exist.** Measured on 2026-09-12, three independent ways:

- **The interface offers no rename.** The operator went looking and found none on the Environment's settings page.
- **The API refuses.** `PATCH` and `POST` on `/repos/{owner}/{repo}/environments/{name}` both return `404`.
- **The documentation lists no such operation.** GitHub's REST reference gives that path only `GET`, `PUT` and `DELETE` — and the name is a path segment, so `PUT` creates or updates the environment it names rather than moving one. GitHub's own "managing environments" page documents creation and deletion and says nothing about renaming.

**This is the stop condition this decision was written to reach**, and it reached it in the cheapest possible place: before the stray Environment was deleted, before a single secret was created, before the HCP renames, and with nothing pushed. The plan said that on this outcome the change returns to the operator with decision 3's rejected alternative and the question it rests on — whether all twenty-one values can be produced. It did, with the answer costed: eighteen recoverable, and three SSH private halves (two converge keys, one platform deploy key) that `docs/bootstrap-a-new-host.md` §0.3 has the operator delete once stored, each needing a rotation with a host-side step. **The operator deferred rather than paid**, and `docs/change-queue.md` entry 75 carries the finding, the price and the two failure modes.

**The stray Environment is therefore left in place rather than deleted.** Entry 75 needs that name, and an empty Environment holding it costs nothing meanwhile — it has no secrets, no rules and no deployments, and no workflow names it.

**What this cost and what it bought.** It cost the eight specification deltas, which were written and withdrawn, and the repository half of the Environment rename, which was implemented and reverted. It bought the discovery before any external state moved — the alternative being to learn it between an approval and a merge, with `production` already deleted or half-copied and a production apply waiting. A probe placed where this one was placed is worth more than the work it discards.
...
The merge also triggers `host-converge.yml`, because `ansible/inventory/*.hcloud.yml` changes. Unlike entry 62's merge there is **no race** with the apply: this change alters no Hetzner label, so the converge's inventory parse does not depend on the apply having landed. What it does depend on is step 1 having happened: it reads `HCLOUD_TOKEN_MAIN_PRODUCTION`, and attaches to an Environment this change does not touch.

**A third pipeline nearly joined them, and the near miss is the thing worth keeping.** `platform-deploy.yml` is path-filtered to `platform/**`, and a round-2 code-review fix corrected an Environment name in `platform/README.md` — bringing this change inside that filter, so its merge would have started a gated production deploy nobody had planned for. It was caught by the review's third round and became a task saying not to approve the resulting request. Deferring the Environment rename then reverted that one-word fix, the file left the diff, and the pipeline stopped being triggered at all; the task went with it.

**Twice in this change, two correct fixes in one commit altered each other's preconditions** — this, and the entry-70 rewrite that defeated decision 4's self-retiring exemption. Both were caught by review rather than by a check, and neither is the kind of thing a check could catch. Recorded because decision 1's whole argument is that this change's window is bounded by an instruction rather than a mechanism, and an instruction list that grows or shrinks silently is what that argument is exposed to.
...
The available treatments were three:
...
**The one delta this change ships is the ninth**, *Remote State Backend*, and it is modified for an unrelated reason: decision 2's measurement makes one of its rationale sentences false. That edit changes no obligation — the requirement still obliges one workspace per stack and no derivation between the names — only the explanation of what pushing in the wrong order costs.
