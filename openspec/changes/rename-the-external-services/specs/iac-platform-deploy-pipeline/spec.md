...
The deploy job SHALL run only after a pull request changing `platform/**` is merged to `main`, and SHALL require manual approval via the same `main-production` GitHub Environment protection rule already used by the Terraform apply workflow.
