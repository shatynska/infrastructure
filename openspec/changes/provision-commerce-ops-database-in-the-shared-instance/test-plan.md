...
`TestTheCodeReviewDetectorsFire` runs each over fixtures that falsify it — for the listings, both removed, one never fed to the check, two merged into one substitution, and the repository check conditioned on `rotate`; and: an indented terminator, the whole block indented under a list item, `<<-` with tabs (accepted) and with spaces (reported), both quoting styles, each log setting removed and one moved after the password, `rotate=no` removed and `${ROTATE:-no}` read from the shell.
