...
Production's private PostgreSQL ends in two steps: the `commerce-ops` repository moves its whole database, queue included, to Supabase and removes its own PostgreSQL service and volume; and the divergence paragraph is deleted here. As first planned it had a third, taken first — provisioning production's shared-instance database for the `procrastinate_*` tables (decision 6) — which the operator's split by host withdrew after it had run (decision 3); that database is unused and its removal is recorded separately.
