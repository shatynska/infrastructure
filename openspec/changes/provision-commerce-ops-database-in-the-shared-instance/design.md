...
**Re-affirmed by the operator on 2026-09-14, against the application's own recorded decisions.** `commerce-ops`'s `docs/environments.md` (2026-09-08, marked there as decisions not yet a change) records the opposite in three places: its decision 4, that the queue cannot leave the domain database because `job_nudge.py` enqueues into `procrastinate_jobs` on the caller's own session inside the domain transaction, a transactional outbox, so that "commerce-ops uses nothing in the shared platform Postgres"; its decision 5, a Supabase project of its own for staging; and its decision 10, staging kept close to production in data. Found after this change merged and asked which holds, the operator chose this change's plan as built. The consequences are that repository's to carry, not this one's: putting the `procrastinate_*` tables in production's shared instance while the durable tables go to Supabase splits that outbox across two databases, which is a modification of its `launch-clickup-sync` specification rather than a deployment choice; staging holds rehearsal data only, which withdraws its decision 10; and its `docs/environments.md` has to be revised to match before its staging or production work is proposed against it.
