...
**Superseded for production by the operator on 2026-09-14, after this change merged.** `commerce-ops`'s `docs/environments.md` (2026-09-08, marked there as decisions not yet a change) records that its queue cannot leave the domain database: `job_nudge.py` enqueues into `procrastinate_jobs` on the caller's own session inside the domain transaction, a transactional outbox, so a queue in a second database would either run work for a rolled-back change or never run work a committed one owed. Asked first, the operator kept this change's plan; asked again with Supabase's connection constraints in hand, the operator chose a split by host. **On staging** the whole of `commerce-ops`'s database stays in the shared instance, as provisioned and under the rehearsal-data policy — one database, so the outbox is intact. **On production** nothing is placed in the shared instance: the whole database, `procrastinate_*` tables included, moves to Supabase, reached through its session pooler — `aws-1-eu-west-1.pooler.supabase.com:5432`, IPv4 only, which answered `pg_isready` from a container on staging's `platform_edge` on 2026-09-14, where Supabase's direct connection is IPv6-only and `platform_edge` has IPv6 disabled. The division above therefore still classifies the tables but places none of them in production's shared instance, and the `commerce-ops` role, database and `SHARED_POSTGRES_PASSWORD` secret this change provisioned there were left unused. That application's decisions 5 and 10 — a Supabase project for staging, and staging kept close to production in data — stay withdrawn for staging, and its `docs/environments.md` is revised to match in that repository.

**Revised again by the operator later on 2026-09-14: production goes to the shared instance first, and to Supabase later.** Production's `commerce-ops` is to leave its private PostgreSQL for the database this change provisioned in production's shared instance — its whole database, queue included, so the outbox stays intact — and move to Supabase only afterwards. Its data includes the hand-curated tables classified above as durable, which *Single Shared PostgreSQL Instance, Per-Application Databases* as it stands keeps out of the shared instance unconditionally. Asked whether to keep the private database until Supabase, or to admit the data with an off-host backup and a rehearsed restore, the operator chose to classify `commerce-ops`'s production data as tolerable to lose, accepting that a volume reset under `upgrade-the-shared-postgres-major` or a host rebuild deletes it. The operator's reason is the application's stage: `commerce-ops` is at a very early, experimental stage, and the classification is temporary, ending with the move to Supabase. That classification is a specification change this change does not make: `docs/backlog.md` `classify-commerce-ops-production-data-for-the-shared-instance` owes it, and no production data may land in the shared instance before it merges. The production role, database and `SHARED_POSTGRES_PASSWORD` are therefore kept for that cutover rather than removed.
...
- **Secret first, then the host, and what each failure leaves.** The reverse order would leave, on a failed secret step, a role whose password exists only in a variable that is about to disappear. With this order:
  - A secret-name listing fails — `gh secret list` for the Environment or the repository, or `gh api` for the secrets the organisation shares with the repository — because the Environment does not exist, `gh` is not authorised, or — for the organisation listing — the repository belongs to a personal account, which GitHub answers with `404` (measured 2026-09-14 against this repository, `shatynska/shatynska-infrastructure`), as GitHub may also answer a token unable to see an organisation's secrets; stage 8.3 tells the operator to make that line `org_names=` only when the repository's `owner.type` is `User`: nothing is set and the host is untouched. Staging was in this state until the operator created its Environment (Context).
  - The name is held by the repository or the organisation: refused whatever `rotate` says, nothing set, host untouched.
  - The name is in the Environment and `rotate` is not `yes`, or `rotate` is `yes` and the name is not in the Environment: refused, nothing set, host untouched.
...
- **A secret name nothing in that Environment already uses.** Production's `commerce-ops` Environment holds `POSTGRES_PASSWORD` for the private PostgreSQL, measured. Setting that name would change what the next production deploy renders for a database that ignores it once initialised — an outage of the durable half, with the overwritten value unrecoverable from GitHub. So the recipe reads the secret names first — the Environment's, the repository's, and those the organisation shares with the repository; names, never values — and **refuses** if the name is in any of them, because an Environment secret overrides a repository or organisation secret of the same name for that environment's jobs, so setting any of the three is an overwrite from the application's side (added at code review, which found the Environment-only check missed the other two). Each listing is its own assignment, because several commands inside one `$(…)` report only the last one's exit status and `set -e` would not see an earlier failure. Names are compared ignoring case (`grep -ixF`), because GitHub's secret names ignore case and a lower-case `<SECRET>` would otherwise slip past an upper-case one — added at code review round 3. A name found at repository or organisation level is refused outright. A name already in the Environment is refused unless `rotate=yes` says this run rotates that very secret, and `rotate=yes` is itself refused when the name is not in the Environment — added at code review round 2, which found that `rotate=yes` had switched the whole check off, in exactly the rebuild and recovery runs where the docs prescribe it. A comment advising the check would be read after the overwrite had already happened, since the block runs whole. For `commerce-ops` that name is `SHARED_POSTGRES_PASSWORD`, the same in both Environments because one workflow renders both; which name the application reads is otherwise that repository's to choose.
...
Production's private PostgreSQL ends, as the operator last decided (decision 3), in four steps: production's shared-instance database is provisioned (decision 6, done); `commerce-ops`'s production data is classified as tolerable to lose, a specification change here; the `commerce-ops` repository moves its whole database, queue included, into that shared-instance database and removes its own PostgreSQL service and volume; and the divergence paragraph is deleted here. The later move to Supabase is not a step of ending this divergence.

This change does not take the last step on the strength of the first. Before archiving, it checks on production: `docker ps --filter name=commerce-ops` shows no PostgreSQL container **and** `docker volume ls` shows `commerce-ops_commerce_ops_pgdata` gone. If both hold, the change re-enters `plan` to add a delta on `iac-safety-hardening` removing that paragraph and the scenario "The stated divergence is not a precedent", with its own derived tests and review. If either does not, the paragraph stays, and tasks.md records the deletion under `## Not performed` with the Supabase backlog entry named as where the remainder lives. A specification claiming the divergence is closed while the container runs is worse than one admitting it.
