...
**One consequence is stated rather than discovered.** `role_names()` can now raise `ManifestNotUsable`. On a tree whose `ansible/requirements.yml` is missing or unparseable, a test resting on it fails naming that file instead of carrying on against a silently different role set. Where the call is in `setUp`, `unittest` reports that as an **error** rather than a failure; the suite goes red either way and the message is the same, and the difference is named here because design.md's account of the failure polarity would otherwise read as uniform when it is not.
