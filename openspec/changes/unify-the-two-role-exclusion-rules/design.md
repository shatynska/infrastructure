...
**The `AssertionError` property is not uniform, and the non-uniformity is stated rather than left to be met.** Several inheriting call sites call the enumeration in `setUp` — `test_ci_configuration.py`'s timer-unit suite, three classes in the matrix module, two in the apt module — and `unittest` reports an `AssertionError` raised in `setUp` as an **error**, not a failure. The suite goes red either way and the message names the manifest either way; an operator reading the output sees errors where a uniform account would predict failures.
