...
**Reason**: Replaced by *Each Stack's Deploy Attaches to the Environment Its Own Declaration Names*, added above, which states the same obligation over every stack rather than over production alone. The name is what made a replacement necessary rather than a modification: it asserts that there is one deploy and that it reuses *the production* Environment, which is precisely what this change removes. A requirement named for production reads as silent about a second stack at exactly the point where a second stack's deploy credential and approval rules are what the confinement protects production from.

The obligation itself is carried through unchanged and generalised: one Environment gates both a stack's Terraform apply and its platform deploy, so that neither mechanism defines an approval list of its own. Both of its scenarios are carried through, one of them widened to say which stack's Environment is meant, and two are added for the cases a single-stack requirement could not express.

**Carried forward, and named so it is not read as dropped**: the non-normative paragraph `rename-the-github-environments` added to this requirement — that the Environment is named for the stack rather than for the environment axis — is reproduced in the added requirement above. It is the sentence `.github/tests` cites for the stack-axis relation, and a rename that silently dropped it would leave that assertion tracing to nothing.

**Migration**: None for production. Its deploy attaches to the same `main-production` Environment, with the same required reviewer, and deploys the same content to the same host. What changes is that the Environment is read from the production stack's own declaration rather than written as a literal on the job, which is the substitution the four Terraform workflows already made.
