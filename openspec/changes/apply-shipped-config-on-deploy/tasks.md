...
- [ ] 1.1 Add a `labels:` entry to `prometheus`, `alertmanager` and `grafana` in `platform/docker-compose.yml`, each holding the digest this change's `design.md` Decision 3 specifies exactly: SHA-256, hex, first 12 characters, over the concatenation of `<config name>\n<content>\n` for each config that service mounts, in mount order, and over that service's configs alone. Verify with `docker compose config --hash='*'` that each of the three hashes now differs from the value on the corresponding running container, and that no other service's hash moved.
- [ ] 1.2 Comment the labels in the style the surrounding file uses: what the digest covers, why it exists — Compose's config-hash does not include inline `configs:` content, and that content is copied into the container at creation rather than bind-mounted, so without this a configuration-only deploy recreates nothing, changes nothing and reports success — and that the static check names the correct value on a mismatch. Verify by reading the diff: a reader who does not know the defect should not have to leave the file.
...
- [ ] 2.5 Assert no service without `configs:` carries the label, so a copy-paste cannot quietly widen what gets recreated. Verify against a copy that adds the label to `postgres`.
...
- [ ] 2.7 Run the whole suite and confirm it is green apart from the assertions section 1 has not yet satisfied.
- [ ] 2.8 Commit the derived tests exactly as their author wrote them, before any folding or relocation, per `docs/change-queue.md` entry 42. Verify the commit contains tests only.

## 3. Bookkeeping

- [ ] 3.1 Add a `docs/change-queue.md` entry for the wider backstop this change deliberately does not build: after `up -d --wait`, compare each running container's `com.docker.compose.config-hash` against `docker compose config --hash='*'` and fail the deploy on a mismatch. Verify the entry records that it does not subsume this change — without a label the two hashes agree while the configuration sits unapplied — and that it touches the generic `app-deploy` script, so it affects every application.
- [ ] 3.2 Record in `platform/README.md` that editing an inline config requires regenerating that service's checksum label, that the static check names the correct value, and that the edit will recreate that service on the next deploy. Verify a reader editing an alert rule would find this before opening a pull request.

## 4. Verification

- [ ] 4.1 Run `python3 -m unittest discover --start-directory .github/tests` from the repository root and confirm every assertion from section 2 passes.
- [ ] 4.2 Run `pre-commit run --all-files` and confirm it is clean. Provision first — a fresh working tree has no Galaxy roles, and the Ansible syntax check fails without them in a way that looks like a defect in this change.
- [ ] 4.3 Run `openspec validate --strict apply-shipped-config-on-deploy` and confirm it passes.
- [ ] 4.4 Confirm against the running host that the three services' current `com.docker.compose.config-hash` values differ from what this branch's file computes, and that no other service's does. This is the pre-merge half of the confirmation and can only be taken before the deploy.
