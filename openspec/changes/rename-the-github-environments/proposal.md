...
- **One credential pair is reissued and everything else is carried over unchanged.** Of the four values the operator found missing from the password manager, three exist elsewhere and are read back: `PLATFORM_ACME_EMAIL` and `PLATFORM_DEPLOY_HOST` off the running platform stack, and `TF_API_TOKEN` from `~/.terraform.d/credentials.tfrc.json`, where `terraform login` put it. Only the **Tailscale OAuth pair** genuinely cannot be read back anywhere, so a new client is created in its admin console — created rather than swapped, because one client serves both Environments and the old one must keep working until the Environments holding it are deleted. No SSH key is rotated and no host's `authorized_keys` is edited.
