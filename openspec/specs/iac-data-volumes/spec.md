...
Lets a stack provision a persistent Hetzner Cloud volume, attached to that stack's server, that can be toggled out of management without losing its declared configuration — mirroring how the server itself is toggled. The volume is named on the rank axis (`main`) rather than for what consumes it, and its on-host device is keyed on the volume's id, so its name and its mount path are independent.
