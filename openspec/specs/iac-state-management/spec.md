...
HCP Terraform backend configuration — the CLI-driven workflow, Local execution mode, one workspace per stack, and a static API token that is deliberately **not** split by privilege — together with the dedicated Hetzner Cloud project each stack operates against, which is where this capability's load-bearing credential boundary actually lives.

This Purpose said "static tokens split by privilege" until 2026-09-12, contradicting the requirement it summarises, whose own title is *Unsplit by Privilege* and which records that split as unavailable on this tier and its absence as an accepted risk. Corrected by `rename-the-external-services`, whose archive step obliges reading a touched capability's Purpose by hand — `openspec archive` reports `→ 0` and leaves every Purpose as it found it.
