...
- **WHEN** a stack's declaration states that the shared platform stack is deployed to it, and the `group_vars` file of the Ansible group it declares does not enumerate the platform application among that host's deploy-key authorisations
...
- **WHEN** a `group_vars` file enumerates the platform application among its host's deploy-key authorisations and that stack's declaration does not opt in to the platform deploy
