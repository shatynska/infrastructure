...
`terraform apply` is never run locally against **any** stack directory under `terraform/stacks/`. Infrastructure changes reach Hetzner only through the gated GitHub Actions pipeline: a PR-time plan for review, then an apply of that exact saved plan on merge to `main`. Local runs use that stack's read-only Hetzner token and are for `terraform plan`/`validate` only.

This holds of every stack, and the stacks differ only in what happens *inside* the pipeline. Prod's apply waits for a human to approve the saved plan; staging's does not, because its GitHub Environment requires no reviewer. That difference is not an exemption from anything above: staging's **Read & Write** token still exists only in its own GitHub Environment's secrets and still never reaches a workstation, and what bounds an unreviewed apply is that the token belongs to a Hetzner project holding staging alone. A stack whose GitHub Environment requires no reviewer is the case this section is most easily read as silent about, so it is named.
...
This is the binding the shared-service rule above promises. Molecule is the one shared service this project's verification writes to, and it shares **three** handles. Two are shared across every working tree on the machine and both are stable per role:
...
The third is shared more widely than either:

- **The loop device minor** that `platform_data_volume`'s fixtures claim. A loop association is made by the machine's **kernel** and is not namespaced by the container that creates it, so it outlives that container and is shared between one working tree's **consecutive runs** as much as between two trees — a tree collides with itself, which no namespace can prevent. A fixture inheriting one inherits whatever the previous run left on it, including a filesystem a previous converge created; for a scenario whose subject is what the role does to an *unformatted* device, that is a pass which exercised nothing.

`~/.cache/molecule/<role>` is **not** a fourth handle under the pinned `molecule==26.8.0`, whatever an older note may say.

**Run the suite through `ansible/scripts/run-molecule`.** It derives this working tree's namespace deterministically from the tree's own path, derives the loop-minor base from that same path, points `ANSIBLE_HOME` at `.molecule-home/` inside the tree, and names the shared `collections` path explicitly. `ansible/scripts/run-molecule --print-namespace` and `--print-loop-base` each compute one and run nothing.
...
**The loop minor is namespaced by derivation and defended by refusal, and it needs both.** The base is `1024 + (digest mod 4096) * 16` over the tree's own absolute path, giving each tree sixteen consecutive minors; each fixture takes a fixed offset, and the two `multiple-devices` arms take separate pairs because both their containers can be up at once. The band starts above the range the kernel allocates from, since `losetup -f` hands back the lowest free minor and a workstation's real loop devices are there. It is not capped at 255 — a device number carries twenty bits of minor, and the loop driver creates a device on first open of any minor below that ceiling rather than only the ones `max_loop` pre-created — so a narrowing of it is a change to check rather than assume.

Because a derived minor is a number nothing outside this repository has agreed to leave alone, and `losetup -d` inside a privileged container reaches the **host's** device, each fixture reads what holds its minor and releases it only where the backing file is one that fixture family creates. Anything else fails the play naming what holds it. That release is unconditional within the namespace, which is what separates a tree's consecutive runs; the namespace is what separates trees. A fixture play started with no base refuses as its first task, before anything has touched the host — a refusing *default* is not available here, because the arithmetic that turns a base into a device path resolves an unset value to `/dev/loop0`, which is a real device.

**One residual, stated rather than managed by a rule.** Four thousand and ninety-six slots admit a collision: two trees whose paths hash to the same slot share every minor in it, and the attribution check above cannot tell the other tree's association from a previous run of your own, so it permits it. One concurrent pair in 4096 running the same role is affected. Nothing in this repository prevents it; the change `namespace-the-molecule-loop-devices`, in its `design.md`, argues why a registry was not worth building for it.
...
Two things about `molecule test --all` that a verification claim depends on. It runs a role's scenarios in sorted order and **stops at the first failure**, so every scenario sorting after a failing one is silently not executed and not listed in the run's SCENARIO RECAP — confirm the recap names every scenario the role has. While a role is red, run its scenarios individually with `ansible/scripts/run-molecule test -s <name>`. Molecule's own `--continue-on-failure` is not available here: it applies only with `--workers`, and `--workers > 1` requires collection mode (`galaxy.yml`), which these plain roles are not.

**Read the exit status, and do not believe the recap's `failed=` count.** This paragraph used to end "rather than reading the exit code alone", which invited exactly the wrong reading. Measured on 2026-09-16 against the pinned `molecule==26.8.0`, with a scenario carrying one deliberately non-idempotent task: the run failed at `idempotence` and **exited 1**, correctly — while its SCENARIO RECAP said `failed=0` and its DETAILS block said `idempotence: Executed: Successful`. Both of those are false, and they are the two signals a reader was being sent to. The exit status was the only honest one.

**A failed action also ends the sequence, so the actions after it never run and are absent from DETAILS.** In that measurement `verify` never appeared, which means the scenario asserted nothing whatever — and a reader who took `failed=0` at face value would have recorded it as a pass. So the check that actually establishes a scenario did its job is that **`verify: Executed: Successful` appears for it in DETAILS**, not the recap's counts and not the exit status alone. Prefer that when reporting a run, and note that `.github/workflows/ansible-verify.yml` reads its own `--all` run by exit status only — which is sound for catching a failure, and says nothing about whether a scenario that is present actually asserted anything.

**Molecule's state is shared across working trees, and three handles carry it — two stable per role, one held by the kernel.** The instance name is a literal in each scenario's `platforms[].name`, so two working trees running the same role create, converge and destroy *the same container*. The ephemeral directory is `$ANSIBLE_HOME/tmp/molecule.<id>.<scenario>` (`~/.ansible/tmp/` by default), and `<id>` is a checksum of the role directory's **basename** rather than of its path — so it is identical across working trees by construction, and relocating a working tree does not escape it. Molecule does not remove that directory when a run finishes, so inheriting one another tree left behind is the default rather than the exception, and `flock` does not help: serialising two runs in time does nothing about a run inheriting a directory left earlier.

One cause, three presentations, which is what makes it read as three unrelated defects: a `create` that fails reading a `molecule.yml` a concurrent `destroy` has just pruned; a `prepare` whose container is torn down under a running play (`UNREACHABLE ... Failed to create temporary directory`); or a `verify` task dying with **rc 137** far into the run. The last is the one most often misread — what identifies it is the pairing of SIGKILL with **empty** stdout and stderr, which Ansible reports as `Module result deserialization failed: No start of json char found`. That reads like a module bug and is not one. `molecule test --all` compounds it, by the rule above: a collision in the first scenario leaves the rest neither executed nor listed. **The danger is not the red runs.** A colliding run can equally pass against a container the other session converged, which reads as evidence that the change under test is sound. **The third handle is the loop device minor**, and it fails differently enough to be worth separating. It is not shared between working trees only — the association is the kernel's and outlives the container, so one tree inherits from *its own* previous run, which is why its fixtures release it unconditionally rather than relying on the namespace alone. Its presentation is the opposite of the ones above: not rc 137 or an `UNREACHABLE`, but a scenario that passes. `platform_data_volume/molecule/default` asserts that the role formats an unformatted device; against an inherited minor the role's format task is a no-op and the scenario is green having exercised nothing. When a device assertion in that role fails in a way that reads as an implementation defect, `losetup -a` **on the host** is the diagnosis — nothing inside the container hints at it.

These handles are namespaced per working tree — see *Namespacing Molecule per working tree* above, which is how you take yours. What is written here is what a run outside that mechanism still meets, and it is worth knowing rather than forgetting: a session's report that it left nothing behind is still worth checking against `docker ps` and `losetup -a` rather than believing.
...
One interval is accepted, and it has two forms. Where a change introduces a **new** capability, archiving is what creates `openspec/specs/<capability>/spec.md`, so a citation of it does not resolve until that change is archived. The same holds where a change adds a **new requirement to an existing** capability and a file it ships cites that requirement by name: the path resolves already, the requirement's own name does not, and archiving is again what closes it. Both are bounded by the change's own life and are not rot — and in both the citation is written in full, with the requirement's name, rather than being weakened to a bare path to avoid the interval. A citation without a requirement name is outside `test_the_retired_requirement_names_are_gone.py`'s sweep, which is a worse trade than the interval.
...
**Scenario titles are not covered by either.** A citation naming a scenario inside a requirement rots the same way and nothing reads it; `docs/backlog.md` `sweep-the-stale-scenario-titles-and-check-them` carries the entry that would.
...
- **Inventory**: dynamic, via the `hcloud` plugin, grouped by the Hetzner labels every resource already carries — one group per axis, `environment` and `tenant` — never a static or hand-maintained hosts file, so a disabled/destroyed server can't leave a stale entry behind. **One source per stack** (`ansible/inventory/<stack>.hcloud.yml`), each naming its own read-only credential variable, because a Hetzner token reaches one project and each stack has a project of its own; adding a stack adds a source rather than editing one. `ansible.cfg` declares no default inventory, so every run names its own with `-i`, and sets `any_unparsed_is_failed` so a source that cannot authenticate fails the run instead of resolving to a group with no host in it.

  **A source is named for its stack and a group is named for its axis, and neither is derived from the other.** They were one word while this repository had one tenant: the stack `prod` was also the group, the `group_vars` filename and the `--vault-id` label. They are not now — the stack is `main-production` and the group is `production` — and the group a converge targets is read from that stack's own `pipeline.yml` (`target_environment`) rather than parsed out of either name. So `group_vars/<environment>.yml`, not `group_vars/<stack>.yml`; and two stacks of different tenants in one environment share one `group_vars` file, which is what an environment-wide baseline means.
