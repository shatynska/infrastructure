...
- **Inventory**: dynamic, via the `hcloud` plugin, grouped by the existing `environment` label — never a static or hand-maintained hosts file, so a disabled/destroyed server can't leave a stale entry behind. **One source per environment** (`ansible/inventory/<environment>.hcloud.yml`), each naming its own read-only credential variable, because a Hetzner token reaches one project and each environment has a project of its own; adding an environment adds a source rather than editing one. `ansible.cfg` declares no default inventory, so every run names its own with `-i`, and sets `any_unparsed_is_failed` so a source that cannot authenticate fails the run instead of resolving to an environment with no host in it.
