...
provider "hcloud" {
  ...
  #     read as `secrets[<the name pipeline.yml declares>]`, and that
  #     declaration is the only place the name is written. A repository
  #     secret holds one value, so each stack needs a read-only secret of
  #     its own.
  #
  #     THE ONE NAME IT MAY NOT BE IS `HCLOUD_TOKEN`, which is a fact
  #     about GitHub rather than about this stack, so it is safe to state
  #     here and is the reason this bullet exists. Every stack's
