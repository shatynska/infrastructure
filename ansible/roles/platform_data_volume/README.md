...
Four Molecule scenarios cover this: `default` (device supplied explicitly), `no-device-discoverable`, and `multiple-devices-discoverable` plus `multiple-devices-reverse-order` — one per directory-read arrangement, since a single arrangement cannot tell a sorted selection from an order-dependent one.
