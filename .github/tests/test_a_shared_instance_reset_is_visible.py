...
#
# THE THIRD ALTERNATIVE IS NOT DECORATION. The recipe destroyed the data with
# `docker volume rm` until move-the-shared-database-onto-the-data-volume put
# the store on the attached volume, where it is cleared with an `rm -rf` inside
# a container instead. Had this pattern kept only the first two alternatives,
# deleting the `docker volume rm` line would have left it matching the
...
DESTRUCTIVE_ACT = re.compile(
    ...
    r"|rm\s+-rf[^\n]*/store"
    r"|docker\s+rm\s+-f\s+platform-postgres-1"
)
