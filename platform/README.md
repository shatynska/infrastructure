...
**`$`, a quote and a backtick are a different defect, still open, and they corrupt the value before the exporter ever sees it** — so this is not a warning about the exporter but about every secret this stack renders. `platform-deploy.yml` writes `.env` with `echo "NAME=${{ secrets.X }}"`, and GitHub substitutes the secret's raw text into that script, so **bash** expands it while building the line — before `.env` exists, and whatever Compose would later do. Measured on 2026-09-15: `ab$c#d` renders as `ab#d`, `"abc"def` as `abcdef`, and a secret containing `` `id -u` `` *runs the command*. The result is a wrong credential, an HTTP 200, `pg_up 0`, a healthy container and no alert — the same silence as before. `docs/backlog.md` `render-the-env-file-so-a-secret-survives-it` is that gap. **Until it closes, generate these with the `openssl rand` commands the bootstrap document gives**, whose alphabets contain none of those characters — a narrower instruction than "avoid `$`", and the right one, since nobody thinks to avoid a backtick.

What `.github/tests` enforces, stated exactly rather than generally: in `platform/docker-compose.yml`, a secret-bearing variable may not appear inside a URL within its own whitespace-delimited word — a scheme, a `user:secret@host` authority, a query parameter, a path segment of a scheme-less authority, or text concatenated onto a secret that is itself a URL. It reads a service's `environment:`, `command:`, `entrypoint:`, `labels:` and `healthcheck.test`, and the inline `configs:` blocks, in both the `${VAR}` and `$VAR` forms. `command:` is not a completeness flourish — it is the only way Traefik's `${ACME_EMAIL}` is reached at all. **Which names count as secrets is read from `platform-deploy.yml`'s render step rather than guessed from keywords**, which is how `DEADMANSWITCH_URL` is covered — a bearer credential whose whole value is a URL and whose name contains no word that says so. A value that is nothing but the secret, as Alertmanager's `api_url` is, is the opposite case and passes. Nothing checks the `.env` rendering above, which is why that paragraph names a backlog entry rather than a check.
