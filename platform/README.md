...
What `.github/tests` enforces, stated exactly rather than generally: in `platform/docker-compose.yml`, a variable whose name carries a secret-ish word may not sit against URL punctuation inside a scheme-bearing URL or a `user:secret@host` authority, in a service's `environment:` or an inline `configs:` block. A value that is nothing but the secret — Alertmanager's `api_url` — is the opposite case and passes. Nothing checks the `.env` rendering above, which is why that paragraph names a backlog entry rather than a check.
