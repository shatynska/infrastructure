...
**`$`, a quote and a backtick were a second defect, in the layer below, and were fixed on 2026-09-15 rather than left as a constraint on what a password may contain.** `platform-deploy.yml` wrote `.env` with `echo "NAME=${{ secrets.X }}"`, and GitHub substitutes a secret's raw text into that script — so **bash** expanded it while building the line, before `.env` existed. Measured: `ab$c#d` rendered `ab#d`, `"abc"def` rendered `abcdef`, and a secret containing `` `id -u` `` *ran the command*, in a job holding that stack's tailnet OAuth client and deploy key. Every value now reaches that script through the step's `env:` block, where the shell never parses it, and `.github/tests` fails the build on any workflow that interpolates a secret — or a workflow input — into a `run:` body.
