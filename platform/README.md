...
- **Compose's own `.env` parser** — **still open**, and it expands `$` in the value whatever the rendering does. Measured from inside a running container on 2026-09-15: a secret stored as `ab$c#d` arrives as `ab#d`, and one beginning with `"` is quote-stripped. Single-quoting the rendered line does not prevent it. `docs/backlog.md` `render-the-env-file-so-a-secret-survives-it` is that gap.

So the standing instruction is unchanged: **generate these with the `openssl rand` commands `docs/bootstrap-a-new-host.md` §0 gives**, whose alphabets contain no `$` and no quote. What the first fix bought is that a value which does get in can no longer *execute*; it can still be silently altered.
