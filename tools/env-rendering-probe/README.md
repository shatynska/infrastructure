...
The image is pinned by digest. Nothing refreshes that pin automatically: Dependabot's Compose fetcher does not descend into subdirectories, and the image-pin assertions in `.github/tests` anchor on `platform/docker-compose.yml` by exact path.
